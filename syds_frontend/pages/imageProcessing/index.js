...
const ImageProcessing = () => {
  const { rawImages, extractedMaps, elements, setElements, setPaddedImages } =
  ...
  const handleLogElements = async () => {
    ...
    const images = handleExtractImages();
    setPaddedImages(images);
  };

  useEffect(() => {
    const loadedElements = [];
    extractedMaps.forEach((b64, idx) => {
      ...
      img.onload = () => {
        ...
        if (loadedElements.length === extractedMaps.length) {
  ...
  const extractElementsAsBase64 = () => {
    ...
    return elements.map((el) => {
      ...
      // Set opacity before drawing
      offscreenCtx.globalAlpha = el.opacity !== undefined ? el.opacity : 1;
      offscreenCtx.drawImage(el.image, el.x, el.y, el.width, el.height);

      // Return base64 of the element at its position
      return offscreenCanvas.toDataURL("image/png"); // You can use "image/png" if transparency is needed
    });
  };
